...
class SQLWord2vec(Functions):
    ...
    def predict(self, spark_session, _model, fname, option):
        model = spark_session.sparkContext.broadcast(_model.getVectors().collect())
        ...
        spark_session.udf.register(fname + "_array", f=f2, returnType=ArrayType(ArrayType(DoubleType())))
        spark_session.udf.register(fname + "_find", f=f3,
                                   returnType=ArrayType(
                                       StructType([
                                           StructField("word", StringType(), False),
                                           StructField("find", DoubleType(), False)
                                       ])))
        spark_session.udf.register(fname, f=f1, returnType=ArrayType(DoubleType()))
